...
def Nice():
    ...
    print('\nChanel "Nice" have %s upcoming event' % (len(df_get)))
...
def get_evet_calendar(events_name):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired:
            ...
            print('The token is expired, file  with token was deleted')
    ...
    try:
        ...
        events_result = service.events().list(calendarId=calendar_id, timeMin=now,
                                              ...
                                              maxResults=number_event, singleEvents=True,
...
def show_event():   
    ...
    while answer not in possible_answers: 
        answer =  'all' #input('Select organization number: ') 
...
def case_event(event_name):
    match event_name:
        ...
        case '':
            ...
            return df
        case _:
            return "Something's wrong with function case_event"
...
def selection():
    try:
        ...
        while answer_exist not in ("y", "n"): 
            answer_exist =  "y" #input("Do you want to see existing events? Enter (y/n): ") 
            if answer_exist == "y": 
        ...
        while answer_upcomin not in ("y", "n"): 
            ...
            if answer_upcomin == "y": 
                ...
                if len(df_upcoming)>0:
                    print('df_exist_event', df_exist_event)
                    try:
                    ...
                    if len(df_new_event)>0:
                        ...
                        while answer_insert not in ("y", "n"): 
                            answer_insert = "y" #input("\nDo you want insert data? Enter (y/n): ") 
                            if answer_insert == "y": 
                                print('\n...function insert runing...')
                                insert_event(df_new_event)
                                status_mess='We insert' + str(len(df_new_event)) + 'new messages'
                            elif answer_insert == "n": 
                                print('Don\'t want')
                            else: 
                                print("Please enter y or n") 
                    else:
                        status_mess='We don\'t insert data, because there are no new events'
                        print(status_mess)
                else:
                    status_mess='We don\'t insert data, because there are no upcoming events'
                    print(status_mess)
                ...
                print("Please enter y or n") 
        status_code='code executed successfully'
...
status_code, status_mess=selection()
...
def telegram_send_messages(status_code, status_mess):
...
telegram_send_messages(status_code, status_mess)
